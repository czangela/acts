...
SeedFilter<external_spacepoint_t>::filterSeeds_2SpFixed(
  ...
  for (size_t i = 0; i < topSpVec.size(); i++) {
    ...
    size_t compatibleSeedCounter = 0;
    ...
    for (size_t j = 0; j < topSpVec.size(); j++) {
      ...
      if (!(std::abs(lastCompatibleSeedR - otherTop_r) < m_cfg.deltaRMin)) {
        ++compatibleSeedCounter;
        lastCompatibleSeedR = otherTop_r;
      }

      if (compatibleSeedCounter >= m_cfg.compatSeedLimit) {
        break;
      }
    }

    weight += compatibleSeedCounter * m_cfg.compatSeedWeight;
